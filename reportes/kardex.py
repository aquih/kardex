...
class kardex_asistente(osv.osv_memory):
    ...
    _columns = {
        ...
        'fecha_inicio': fields.datetime('Inicio', required=True),
        'fecha_fin': fields.datetime('Fin', required=True),
    ...
    _defaults = {
         ...
         'fecha_inicio': lambda *a: time.strftime('%Y-%m-01 06:00:00'),
    }

    def inicial(self, cr, uid, datos, context=None):

        cr.execute("select sum(qty_in) as entrada, sum(qty_out) as salida, product_id \
            from ( \
            ...
            group by product_id",
            (datos.producto.id, datos.ubicacion.id, datos.fecha_inicio, datos.producto.id, datos.ubicacion.id, datos.fecha_inicio))
    ...
    def lineas(self, cr, uid, datos, context=None):
        ...
        totales['inicio'] = self.inicial(cr, uid, datos, context=context)
        ...
        for m in self.pool.get('stock.move').browse(cr, uid, movimientos_ids, context=context):
            ...
            costo = self.pool.get("product.template").get_history_price(cr, uid, datos.producto.product_tmpl_id.id, m.company_id.id, date=m.date, context=context)
            linea['costo'] = costo
    ...
    def excel_resumen(self, cr, uid, ids, context=None):
        ...
        for a in self.browse(cr, uid, ids):
            ...
            for l in lineas:
                ...
                ctx.update({ 'location': a.ubicacion.id, 'to_date': a.fecha_inicio })
                producto = self.pool.get('product.product').browse(cr, uid, datos.producto.id, context=ctx)
